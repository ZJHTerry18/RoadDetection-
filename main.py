...
parser.add_argument('--rzw', type=float, nargs=2, default=[1,1], help='resize scale of image width.')
parser.add_argument('--rzh', type=float, nargs=2, default=[1,1], help='resize scale of image height')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        model = models.__dict__[args.arch](pretrained=True, auxiliary=args.conloss, num_classes=args.num_classes, multi_cls=args.multi_cls, multi_scale=args.multi_scale)
    else:  
        print("=> creating model '{}'".format(args.arch))
        model = models.__dict__[args.arch](auxiliary=args.conloss, num_classes=args.num_classes, multi_cls=args.multi_cls, multi_scale=args.multi_scale)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if not args.old_version:
                ...
                model.load_state_dict(checkpoint['state_dict'], strict=(not args.evaluate))
                ...
                warnings.warn('You are using old checkpoint, there is no best_auc is recorded.')
                ...
                try:
                    ...
                    try:
                        ...
                        assert args.evaluate, 'this checkpoint is only support evaluate.'
                    print("=> loaded checkpoint '{}' (epoch {})"
                        .format(args.resume, checkpoint['epoch']))
                except:
                    assert args.evaluate, 'this checkpoint is only support evaluate.'
...
def train_one_epoch_with_conloss(train_loader, model, criterion, optimizer, epoch, args, ngpus_per_node):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.rzw != [1,1] or args.rzh != [1,1]:
            randw1 = int((np.random.rand() * (args.rzw[1] - args.rzw[0]) + args.rzw[0]) * args.img_size[1] + 0.5)
            randw2 = int((np.random.rand() * (args.rzw[1] - args.rzw[0]) + args.rzw[0]) * args.img_size[1] + 0.5)
            randh1 = int((np.random.rand() * (args.rzh[1] - args.rzh[0]) + args.rzh[0]) * args.img_size[0] + 0.5)
            randh2 = int((np.random.rand() * (args.rzh[1] - args.rzh[0]) + args.rzh[0]) * args.img_size[0] + 0.5)
            resize1 = transforms.Resize((randh1, randw1))
            resize2 = transforms.Resize((randh2, randw2))
            images[0] = resize1(images[0])
            images[1] = resize2(images[1])
