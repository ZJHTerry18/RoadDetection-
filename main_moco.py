...
import models.efficientnet_pretrain as ourmodels
import torch
...
import moco.loader
import moco.builder
from data import build_transform, build_special_transform, RoadAllData
import cv2
import numpy as np

model_names = sorted(name for name in models.__dict__
    if name.islower() and not name.startswith("__")
    and callable(models.__dict__[name])) + ['eff_v2_s', 'eff_v2_m', 'eff_v2_l']

parser = argparse.ArgumentParser(description='PyTorch ImageNet Training')
parser.add_argument('--data', metavar='DIR', default='/data2/zrliu/dataset_cmp/dataset/', help='path to dataset')

parser.add_argument('--save_path', type=str, default='/data2/zrliu/checkpoints/hwcmp/', 
...
parser.add_argument('--flip', type=str, nargs='+', default=['h', 'v'], help='whether to use rand flip("h" : horizon flip | "v" : vertical flip).')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch.startswith('resnet'):
        model = moco.builder.MoCo(
            ...
            args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.mlp, pretrained=args.pretrained)
    elif args.arch.startswith('eff'):
        model = moco.builder.MoCo(
            ourmodels.__dict__[args.arch],
            args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.mlp, pretrained=args.pretrained)
    print(model)
    ...
    augmentation = build_transform(size=(args.img_size[0], args.img_size[1]),  
                                    updown_rand_crop=args.updown_crop, 
                                    translate=args.translate, 
                                    rate_for_crop=args.crop_rate,
                                    rand_filp=args.flip, to='Tensor', norm=True, canny=args.canny)
    ...
    train_dataset = RoadAllData(args.data, transform=augmentation, 
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, ngpus_per_node=ngpus_per_node)
        ...
        if (not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best=False, basepath=args.save_path, filename='checkpoint_{:04d}.pth.tar'.format(epoch))
...
def train(train_loader, model, criterion, optimizer, epoch, args, ngpus_per_node):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        if args.show and (args.rank % ngpus_per_node == 0):
            img1 = images[0][0].permute(1,2,0).detach().cpu().numpy().copy()
            img2 = images[1][0].permute(1,2,0).detach().cpu().numpy().copy()
