...
def evaluate(args):
    ...
    scores_list, labels = [], []
    for k, (model, name, dataloader) in enumerate(zip(model_list, name_list, dataloader_list)):
        ...
        with torch.no_grad():
            scores_tta = []
            print('=> inference using model {}'.format(name))
            for t in range(args.tta):
                ...
                for i, (images, target) in enumerate(dataloader):
                    ...
                    # compute output
                    outs, _ = model(images)
                    scores.extend(outs[0].detach().cpu().tolist())
                    ...
                    if i % args.print_freq == 0:
                        progress.display(i)
                print("acc: ", top1.avg)
                scores_tta.append(torch.Tensor(scores))
            
            scores_list.append(sum(scores_tta) / len(scores_tta))
    
    labels = torch.Tensor(labels)
    if args.softmean:
        soft_list = []
        for scores in scores_list:
            soft_list.append(scores.softmax(dim=1))
        ps = sum(soft_list) / len(soft_list)
    
    else:
        ps = (sum(scores_list) / len(scores_list)).softmax(dim=1)
...
def submission(args):
    ...
    scores_list, name_list = [], []
    for k, (model, dataloader) in enumerate(zip(model_list, dataloader_list)):
        model.eval()
        with torch.no_grad():
            scores_tta = []
            for t in range(args.tta):
