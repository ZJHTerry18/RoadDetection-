...
# 网络训练相关超参数
parser.add_argument('-a', '--archs', nargs='+', default=['resnet34'], choices=model_names, help='model architecture: ' +' | '.join(model_names) +' (default: resnet18)')
...
parser.add_argument('--checkpoints', nargs='+', type=str, default='', help='checkpoint list for ensmble models.')
parser.add_argument('--resume', nargs='+', type=str, choices=['p', 'm', 'r', 'n'], default=['r'], help="mode for loading checkpoint, 'p' for pretrain, 'm' for moco, 'r' for resume.")
# parser.add_argument('--moco_pretrained', type=str, default='', help='checkpoint path of moco pretrained model.')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for i in range(num_models):
        if 'p' in args.resume[i]:
            print("=> using pre-trained model '{}'".format(args.arch))
            models[i] = ourmodels.__dict__[args.arch](pretrained=True, auxiliary=args.conloss, num_classes=args.num_classes, multi_cls=args.multi_cls, multi_scale=args.multi_scale)
        else:  
            print("=> creating model '{}'".format(args.arch))
            models[i] = ourmodels.__dict__[args.arch](auxiliary=args.conloss, num_classes=args.num_classes, multi_cls=args.multi_cls, multi_scale=args.multi_scale)
        print("model has %.2fM trainable parameters in total." % (count_parameters(model) / 1e6))

        if 'm' in args.resume[i]:
            checkpoint = torch.load(args.checkpoints[i])
            ...
            for k in list(state_dict.keys()):
                ...
                del state_dict[k]
            msg = model[i].load_state_dict(state_dict, strict=False)
            flag = True
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            for model in models:    model.cuda(args.gpu)
            ...
            if args.unlabeled:
                for model in models:    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu], broadcast_buffers=False, find_unused_parameters=True)
            else:
                for model in models:    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu])
        else:
            for model in models:    model = model.cuda()
            # DistributedDataParallel will divide and allocate batch_size to all
            # available GPUs if device_ids are not set
            if args.unlabeled:
                for model in models:    model = torch.nn.parallel.DistributedDataParallel(model, broadcast_buffers=False, find_unused_parameters=True)
            else:
                for model in models:    model = torch.nn.parallel.DistributedDataParallel(model)

    elif args.gpu is not None:
        torch.cuda.set_device(args.gpu)
        for model in models:    model = model.cuda(args.gpu)
    else:
        ...
        if args.arch.startswith('alexnet') or args.arch.startswith('vgg'):
            ...
            for model in models:    model = torch.nn.DataParallel(model).cuda()
    ...
    optimizers = [None for i in range(num_models)]
    for i in range(num_models):
        if args.optimizer == 'AdamW':
            print('Optimizier: AdamW')
            optimizers[i] = torch.optim.AdamW(model.parameters(), lr=args.lr, betas=(0.9, 0.999), weight_decay=args.weight_decay)
        elif args.optimizer == 'SGD':
            optimizers[i] = torch.optim.SGD(model.parameters(), args.lr,
    ...
    for i in range(num_models):
        if 'r' in args.resume:
            if os.path.isfile(args.checkpoints[i]):
                print("=> loading checkpoint '{}'".format(args.checkpoints[i]))
                if args.gpu is None:
                    checkpoint = torch.load(args.checkpoints[i])
                else:
                ...
                models[i].load_state_dict(checkpoint['state_dict'], strict=(not args.evaluate))
                # if not args.evaluate:
                #     optimizer.load_state_dict(checkpoint['optimizer'])
                print("=> loaded checkpoint '{}' (epoch {})"
                    .format(args.checkpoints[i], checkpoint['epoch']))
    ...
    transform_train = build_transform(size=(args.img_size[0], args.img_size[1]), rate_for_crop=args.crop_rate, rand_filp=args.flip, to='Tensor', norm=True, 
        updown_rand_crop=args.updown_crop, translate=args.translate, canny=args.canny, hist=args.hist, hist_level=args.hist_level, colorjittor=args.colorjittor)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        if args.conloss:
            train_one_epoch_with_conloss(train_loader, models, criterion, optimizers, epoch, args, ngpus_per_node)
        else:
            train_one_epoch_without_conloss(train_loader, models, criterion, optimizers, epoch, args, ngpus_per_node)
        
        # evaluate on validation set
        acc1, auc = validate(val_loader, models, criterion, args)
...
def train_one_epoch_without_conloss(train_loader, model, criterion, optimizer, epoch, args, ngpus_per_node):
    ...
    progress = ProgressMeter(
    len(train_loader),
    [batch_time, data_time, losses_ce, losses_tri, losses_logtri, top1],
    prefix="Epoch: [{}]".format(epoch))
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        loss = loss_ce + args.tri * loss_tri + args.logtri * loss_logtri
        # measure accuracy and record loss
...
def validate(val_loader, models, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            outs, loss_tri = [[],[],[],[],[],[],[],[]], []
            for model in models:
                outs_, feats_ = model(images)
                loss_tri_, _ = tri_criterion_(feats_, target[:,0])
                for j in range(num_classes):    outs[j].append(outs_[j])
                loss_tri.append(loss_tri_)
            for j in range(num_classes):    outs[j] = sum(outs[j]) / len(outs[j])
