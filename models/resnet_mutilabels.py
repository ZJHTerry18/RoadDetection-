...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.feature_dim = 640 if multi_scale else 512
        ...
        if multi_cls:
            self.fc = nn.ModuleList([nn.Linear(self.feature_dim * block.expansion, 2), nn.Linear(self.feature_dim * block.expansion, 8)])
        else:
            self.fc = nn.ModuleList([nn.Linear(self.feature_dim * block.expansion, 2) for _ in range(num_classes)])
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        out1_feature = self.auxiliary4(feature_list[0]).view(x.size(0), -1)
        out2_feature = self.auxiliary4(feature_list[1]).view(x.size(0), -1)
        out3_feature = self.auxiliary4(feature_list[2]).view(x.size(0), -1)
        out4_feature = self.auxiliary4(feature_list[3]).view(x.size(0), -1)

        if self.multi_scale:
            out4_feature = torch.cat([out2_feature, out4_feature], dim=1)
        
        outs = []
        for i in range(len(self.fc)):
            out = self.fc[i](out4_feature)
            outs.append(out)
        ...
        if self.training and self.auxiliary:
            ...
            return outs, feat_list
        else:
            return outs, out4_feature
