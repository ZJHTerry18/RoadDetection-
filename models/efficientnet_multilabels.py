...
class EffB7Model(nn.Module):
    def __init__(self, pretrained=False, auxiliary=False, num_classes=8, multi_cls=False, multi_scale=False):
        ...
        self.fc = nn.ModuleList([nn.Linear(2560, 2) for _ in range(self.num_classes)])
    ...
    def forward(self, x):
        ...
        if self.training and self.auxiliary:
            return outs, [F.normalize(feature, dim=1)]
        else:
            return outs, feature
...
class EffV2SModel(nn.Module):
    def __init__(self, pretrained=False, auxiliary=False, num_classes=8, multi_cls=False, multi_scale=False):
        ...
        self.backbone.classifier = torch.nn.Sequential()
        self.fc = nn.ModuleList([nn.Linear(1280, 2) for _ in range(self.num_classes)])
